class PropertiesPanel {
    ...
    updateForSelectedObject() {
        ...
        const oldMessage = this.shapeFolder.domElement.querySelector('.no-selection-message');
        if (oldMessage) {
            oldMessage.remove();
        }
        ...
        if (!this.engine.selectedObject) {
            ...
            noSelectionMsg.className = 'no-selection-message';
    ...
    addShapeControllers() {
        ...
        this.controllers.shape.x = this.shapeFolder.add(obj, 'x', 0, this.engine.canvasWidth).step(1);
        this.controllers.shape.y = this.shapeFolder.add(obj, 'y', 0, this.engine.canvasHeight).step(1);
        this.controllers.shape.rotation = this.shapeFolder.add(obj, 'rotation', 0, 360).step(1);
        
        // Shape-specific properties
        if (obj instanceof Circle) {
            this.controllers.shape.width = this.shapeFolder.add(obj, 'width', 1, 500).step(1).name('diameter');
        } 
        else if (obj instanceof Rectangle) {
            this.controllers.shape.width = this.shapeFolder.add(obj, 'width', 1, 500).step(1);
            this.controllers.shape.height = this.shapeFolder.add(obj, 'height', 1, 500).step(1);
            this.controllers.shape.cornerRadius = this.shapeFolder.add(obj, 'cornerRadius', 0, 100).step(1);
        } 
        else if (obj instanceof Text) {
            this.controllers.shape.text = this.shapeFolder.add(obj, 'text');
            this.controllers.shape.fontSize = this.shapeFolder.add(obj, 'fontSize', 8, 72).step(1);
    ...
    addStyleControllers() {
        ...
        this.controllers.style.opacity = this.styleFolder.add(obj, 'opacity', 0, 255).step(1);
        this.controllers.style.strokeWeight = this.styleFolder.add(obj, 'strokeWeight', 0, 20).step(0.5);
        this.controllers.style.visible = this.styleFolder.add(obj, 'visible');
        
        // Add additional style options
        if (obj instanceof Text) {
            this.controllers.style.letterSpacing = this.styleFolder.add(obj, 'letterSpacing', -5, 20).step(0.5);
            this.controllers.style.lineHeight = this.styleFolder.add(obj, 'lineHeight', 0.5, 2).step(0.1);
        }
        
        // Add fill and stroke options
        const fillOptions = { fillType: obj.fillType || 'solid' };
        this.controllers.style.fillType = this.styleFolder.add(fillOptions, 'fillType', ['solid', 'gradient', 'none']).onChange(value => {
            obj.fillType = value;
    ...
    addAnimationControllers() {
        ...
        this.controllers.animation.previewAnimation = this.animationFolder.add(animationActions, 'previewAnimation').name('Preview');
    ...
    addAnimationPresets() {
        ...
        const presets = {
            fadeIn: () => {
                ...
                this.keyframeManager.createKeyframe(obj, 'opacity', 24, 255, 'easeOutCubic');
            },
            fadeOut: () => {
                const startFrame = prompt('Start frame for fade out:', this.engine.timeline.currentFrame);
                if (startFrame) {
                    const frame = parseInt(startFrame);
                    this.keyframeManager.createKeyframe(obj, 'opacity', frame, 255);
                    this.keyframeManager.createKeyframe(obj, 'opacity', frame + 24, 0, 'easeInCubic');
                }
            },
            bounceIn: () => {
                this.keyframeManager.createKeyframe(obj, 'y', 0, obj.y + 100);
                this.keyframeManager.createKeyframe(obj, 'y', 10, obj.y - 20, 'easeOutQuad');
                this.keyframeManager.createKeyframe(obj, 'y', 14, obj.y + 10, 'easeInQuad');
                this.keyframeManager.createKeyframe(obj, 'y', 18, obj.y - 5, 'easeOutQuad');
                this.keyframeManager.createKeyframe(obj, 'y', 22, obj.y, 'easeInOutQuad');
            },
            pulse: () => {
                const baseScale = 1.0;
                ...
                if (obj instanceof Circle || obj instanceof Rectangle) {
                    ...
                    // Width pulsing
                    this.keyframeManager.createKeyframe(obj, 'width', currentFrame, baseWidth);
                    this.keyframeManager.createKeyframe(obj, 'width', currentFrame + 10, baseWidth * 1.2, 'easeOutQuad');
                    this.keyframeManager.createKeyframe(obj, 'width', currentFrame + 20, baseWidth, 'easeInQuad');
                    
                    // Height pulsing for rectangles
                    if (obj instanceof Rectangle) {
                        this.keyframeManager.createKeyframe(obj, 'height', currentFrame, baseHeight);
                        this.keyframeManager.createKeyframe(obj, 'height', currentFrame + 10, baseHeight * 1.2, 'easeOutQuad');
                        this.keyframeManager.createKeyframe(obj, 'height', currentFrame + 20, baseHeight, 'easeInQuad');
                    }
                }
            },
            spin: () => {
                ...
                this.keyframeManager.createKeyframe(obj, 'rotation', currentFrame, currentRotation);
                this.keyframeManager.createKeyframe(obj, 'rotation', currentFrame + 30, currentRotation + 360, 'easeInOutCubic');
            },
            moveRight: () => {
                ...
                this.keyframeManager.createKeyframe(obj, 'x', currentFrame + 30, currentX + 200, 'easeInOutQuad');
            }
        };
    ...
    showPointEditor(pathObj) {
        ...
        editorContainer.className = 'point-editor-container';
        ...
        editorContainer.style.backgroundColor = 'var(--panel-bg)';
        editorContainer.style.border = '1px solid var(--border-color)';
        editorContainer.style.borderRadius = '4px';
        editorContainer.style.boxShadow = '0 4px 8px rgba(0,0,0,0.3)';
        ...
        pointsList.style.maxHeight = '300px';
        pointsList.style.overflowY = 'auto';
        pointsList.style.marginBottom = '10px';
        ...
        pathObj.points.forEach((point, index) => {
            ...
            pointItem.style.padding = '5px';
            pointItem.style.border = '1px solid #333';
            pointItem.style.borderRadius = '3px';
            
            const pointNumber = document.createElement('span');
            pointNumber.textContent = `Point ${index+1}:`;
            pointNumber.style.marginRight = '10px';
            pointNumber.style.width = '70px';
            ...
            xInput.style.backgroundColor = '#222';
            xInput.style.color = 'var(--text-color)';
            ...
            yInput.style.backgroundColor = '#222';
            yInput.style.color = 'var(--text-color)';
            yInput.style.border = '1px solid #444';
            yInput.style.borderRadius = '3px';
        ...
        controls.className = 'point-editor-controls';
    ...
    addKeyframeButtonsToFolder(folder, controllers) {
        ...
        for (const propName in controllers) {
            ...
            if (typeof this.engine.selectedObject[propName] === 'number') {
                ...
                keyframeButton.style.color = 'var(--keyframe-color)';
